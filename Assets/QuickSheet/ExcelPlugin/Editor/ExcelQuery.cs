...
/// <summary>
/// Query each of cell data from the given excel sheet and deserialize it to the ScriptableObject's
/// data array.
/// </summary>
public class ExcelQuery {
	...
	public ExcelQuery(string path, string sheetName = "") {
		try {
			...
			Debug.LogError("Exception while opening excel file:");
			Debug.LogException(e);
		}
	}

	/// <summary>Determine whether the excel file is successfully read in or not.</summary>
	public bool IsValid() {
	...
	/// <summary>Retrieves file extension only from the given file path.</summary>
	...
	/// 
	/// NOTE: The first row of a sheet is header column which is not the actual value so it skips
	...
	/// <summary>Retrieves all sheet names.</summary>
	...
	protected object ConvertFrom(ICell cell, Type t) {
		...
		if (t == typeof(float) || t == typeof(double) || t == typeof(int))
			...
			// HACK: handles the case that a cell contains numeric value but a member field in a
			//       data class is defined as string type.
		...
		if (t.IsEnum) {
			...
			// for array type, return comma separated string then parse and covert its corresponding type.
